        ...
        <div class="max-w-screen-xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.webcams.update', $webcam->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ $webcam->name }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">Descrizione</label>
                    <textarea name="description" id="description" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>{{ $webcam->description }}</textarea>
                </div>
                <div>
                    <label for="stream_url" class="block text-sm font-medium text-gray-700">Stream URL</label>
                    <input type="text" name="stream_url" id="stream_url" value="{{ $webcam->stream_url }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
                <div>
